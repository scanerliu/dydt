...
class DBManage
{
    ...
    function backup($tablename = '', $dir, $size)
    {
		/*记录备份时间 beg*/
	    $time=time();
		 mysql_query ( "UPDATE  `pharmacy`.`system` SET  `db_save_time` =  $time WHERE  `system`.`system_id` =1" );
		/*记录备份时间 end*/
		
        $dir = $dir ? $dir : 'backup/';
        $size = $size ? $size : 200000;
        $sql = '';
        // 只备份某个表
        if (! empty ( $tablename ))
        {
...
/*刚代码数据库导入存在问题*/
$db = new DBManage ( 'localhost', 'root', 'root33068', 'pharmacy', 'utf8' ); 
$db->backup (); 
 ?>
</div>
<?php
echo 'Tables   export  successfully';
 ?>
