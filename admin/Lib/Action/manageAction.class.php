 ...
 //            $list[$i]['user_name'] = M('user')->where('user_id='.$list[$i]['user_id'])->getfield('name');
        ...
        public function saveaptitude()
	{
       ...
       public function yanzhengma($sms) /*短信验证码*/ {
            ...
            $ch = curl_init("http://www.10086x.com/sends.asp?user=zsmyzm1&passw=zsm123456&text=" . $text . "&mobiles=" . $sms['tel'] . "&SEQ=1000");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // 获取数据返回
            curl_setopt($ch, CURLOPT_BINARYTRANSFER, true); // 在启用 CURLOPT_RETURNTRANSFER 时候将获取数据返回
            $output = curl_exec($ch);
            Log::write("短信发送：http://www.10086x.com/sends.asp?user=zsmyzm1&passw=zsm123456&text=" . $text . "&mobiles=" . $sms['tel'] . "&SEQ=1000 ,result =".$output, Log::ERR);
        }
