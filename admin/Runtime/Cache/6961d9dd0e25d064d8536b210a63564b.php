...
<div class="bannerList">
  ...
  <table width="99%" border="0" cellpadding="0" cellspacing="0" style=" margin-bottom:5px;" >
    ...
    <?php if(is_array($newlist)): $i = 0; $__LIST__ = $newlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
        <td height="30"><?php echo ($vol["gold_id"]); ?></td>
        <td><input name="" type="checkbox" value="<?php echo ($vol["gold_id"]); ?>"  /></td>
        <td><a   href="/Uploads/<?php echo ($vol["gold_id"]); ?>" target="_new" ><img src="/Uploads/<?php echo ($vol["gold_pic"]); ?>" width="120" /></a></td>
        <td><?php echo ($vol["gold_name"]); ?></td>
        <td><a href="/goldcontent/goldcontent/id/<?php echo ($vol["gold_id"]); ?>" target="_blank" style="color:#06C">链接</a></td>
        <td><?php echo (date('Y年m月d日 H:i:s',$vol["time"])); ?></td>
        <td><a href="__APP__/liu/arcchange/article_id/<?php echo ($vol["article_id"]); ?>" style="color:#03C">修改</a>&nbsp;<a href="__APP__/liu/goldDel/id/<?php echo ($vol["gold_id"]); ?>" style="color:#03C">删除</a></td>
