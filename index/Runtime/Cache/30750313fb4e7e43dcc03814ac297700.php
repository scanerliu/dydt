...
<head>
<meta charset="UTF-8">
<title>我的订单-<?php echo ($top_content["title"]); ?></title>
...
<div class="nav_index_banner">
  ...
  <div class="my_fill">
    ...
    <script type="text/javascript">
	   <?php if($_GET['status']): ?>$('.Lfill_a a').eq(<?php echo ($_GET['status']); ?>).attr('class','Lfill_a1')
		<?php else: ?> $('.Lfill_a a').eq(0).attr('class','Lfill_a1')<?php endif; ?>
    </script>
      <div class="Lfill_search">
        <form action="/order/orderList" method="get" name="order_search">
        <input type="text" name="order_key_word" value="<?php echo ($_GET['order_key_word']); ?>">
    ...
    <ul>
      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><li>
          <div class="resheet_head">
            <label>&nbsp;&nbsp;<?php echo (date('Y年m月d日 H:i:s',$vol["time"])); ?></label>
            <label>订单号：<span><?php echo ($vol["order_num"]); ?></span> &nbsp;&nbsp;  运费 <span style="color:#F00; font-size:16px"><?php echo ($vol["express_fee"]); ?></span>元 &nbsp;&nbsp;&nbsp;总价(包含运费) <span style="color:#F00; font-size:16px"><?php echo ($vol["total"]); ?></span>元</label>
          </div>
          <?php if(is_array($vol["product"])): $i = 0; $__LIST__ = $vol["product"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol2): $mod = ($i % 2 );++$i;?><dl class="reget_box">
              <dt> <a href="/shop/content/product_id/<?php echo ($vol2["product_id"]); ?>"> <img src="/Uploads/<?php echo ($vol2["img"]); ?>"  />
                <div>
                  <h3><?php echo ($vol2["title"]); ?></h3>
                  <p><?php echo ($vol2["subhead"]); ?></p>
                </div>
                </a> </dt>
              <dd class="wid2">
                <div style="margin-top:50px;"><?php echo ($vol2["price"]); ?></div>
              </dd>
              <dd class="num wid2"> <?php echo ($vol2["product_num"]); ?> </dd>
              <dd class="godds_operate"> 
                <?php if($vol["status"] == 1 ): ?><a href="/order/orderListFunorderDel/order_id/<?php echo ($vol["order_id"]); ?>">取消订单</a>
                <?php elseif($vol["status"] == 2 and $vol2["is_return"] != 1 ): ?>
                  <a href="/return/return_one/order_id/<?php echo ($vol["order_id"]); ?>/product_id/<?php echo ($vol2["product_id"]); ?>">退款/退货</a>
               <?php elseif($vol["status"] != 2 and $vol["status"] != 1 and $vol2["is_return"] != 1 ): ?>
                  <a href="/return/return_one/order_id/<?php echo ($vol["order_id"]); ?>/product_id/<?php echo ($vol2["product_id"]); ?>">退款/退货</a><?php endif; ?>
                   <?php if($vol2["is_return"] == 1): ?><a href="/return/return_two/order_id/<?php echo ($vol["order_id"]); ?>/product_id/<?php echo ($vol2["product_id"]); ?>">退款/退货处理中</a><?php endif; ?>
                   <?php if($vol2["is_return"] == 2): ?>退货已经完成<?php endif; ?>
                  <?php if($vol2["is_have_comment_right"] == 1 ): ?><a href="/shop/content/product_id/<?php echo ($vol2["product_id"]); ?>/order_id/<?php echo ($vol["order_id"]); ?>" style="margin-left:9px">去评价</a><?php endif; ?>
                   </dd>
              <dd class="wid2">
                <div style="margin-top:50px;"><?php echo ($vol2["total_price"]); ?></div>
              </dd>
              <dd class="operatea">
                <div style=" margin-top:50px;">
                  <?php if($vol["status"] == 1): ?>待付款
                  <?php elseif($vol["status"] == 2): ?>待发货 
                  <?php elseif($vol["status"] == 3): ?>待确认 
                  <?php elseif($vol["status"] == 4): ?>
                     <?php if($vol2["is_have_comment_right"] == 1): ?>未评价
                       <?php else: ?> 已评价<?php endif; ?>  
                   <?php elseif($vol["status"] == 5): ?>已经评价<?php endif; ?>
                 </div>
              </dd>
              <dd class="now_ha">
                 <div style="margin-top:40px;">
                 <a href="/order/orderDetail/order_id/<?php echo ($vol["order_id"]); ?>" class="re_a1">查看详情</a>
                  </div>
              </dd>
