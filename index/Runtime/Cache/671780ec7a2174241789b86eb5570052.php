...
<head>
<meta charset="UTF-8">
<title>购物车-<?php echo ($top_content["title"]); ?></title>
...
<div class="nav_index_banner">
  ...
  <form class="buy_car">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dl class="car_box">
        <dt> <a href="/shop/content/product_id/<?php echo ($vo["product_id"]); ?>"><img src="/Uploads/<?php echo ($vo['img']); ?>"   />
          <div>
            <h3><?php echo ($vo["title"]); ?></h3>
            <p><?php echo ($vo["subhead"]); ?></p>
          </div>
          </a> </dt>
        <dd><span>￥</span><span id="pr"><?php echo ($vo["price2"]); ?></span></dd>
        <input name="" type="hidden" value="<?php echo ($vo["product_id"]); ?>" class="pro_id">
         <dd>
          <input class="car_btnl" type="button" value="-" />
          <input class="car_text num" type="text" name="" id="num" value="<?php echo ($vo['num']); ?>" />
          <input class="car_btnr" type="button" value="+" />
        </dd>
        <dd>
          <p id="mo"><?php echo ($p); ?></p>
        </dd>
        <dd> <a href="<?php echo U('ordermanage/add_mycol',array('id' => $vo['product_id']));?>">移入收藏夹</a> <a href="<?php echo U('ordermanage/del_shop_cart',array('id' => $vo['product_id']));?>">删除</a> </dd>
